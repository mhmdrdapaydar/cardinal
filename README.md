...
- **Ground detail.** The floor is the largest and emptiest surface on screen in both realms, so `tools/make_ground_decal.py` paints one organic patch — damp staining, branching hairline cracks, moss and grit — and it is scattered as flat planes with varied rotation, scale and opacity, so a single texture reads as many. The wild copies sample the build's own terrain function, so they sit on the hills instead of floating over them. Nothing solid, so nothing needs a collider.
- **Market clutter.** Crates, barrels, grain sacks and a hanging lantern around the stalls. The market already owns a collider circle of radius 4.1, and everything is dropped inside it, so the avatar can no more walk through a barrel than it could through the stall.
- **Glowing mushrooms** in the wild, at ankle height so walking through one is not noticeable and the build's collision volumes stay untouched.
